...
from src.core.config import Settings, get_settings, reload_settings, configure_logging
...
class TestSettings:
    ...
    def test_allow_origins_validation(self) -> None:
        ...
        # Valid origins
        settings = Settings(allow_origins=["http://localhost:3000", "https://example.com"])
        assert settings.allow_origins == ["http://localhost:3000", "https://example.com"]
...
class TestConfigureLogging:
    ...
    @patch('logging.config.dictConfig')
    @patch('logging.getLogger')
    def test_configure_logging_custom_settings(self, mock_get_logger, mock_dict_config) -> None:
