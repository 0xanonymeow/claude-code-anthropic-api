...
class TestErrorHandler:
    ...
    def test_create_validation_error_response(self, error_handler, mock_request):
        ...
        assert isinstance(response, JSONResponse)
        assert response.status_code == 422  # UNPROCESSABLE_ENTITY for validation errors
    ...
    def test_debug_mode_error_details(self):
        ...
        production_response, _ = (
            production_handler.mapper.map_exception_to_error_response(
                error, include_details=False
            )
        )
    ...
    def test_error_response_serialization(self):
        ...
        error_response = ErrorResponse(
            error=AnthropicError(
                type=ErrorType.INVALID_REQUEST_ERROR, message="Test error message"
            )
        )
...
class TestAsyncErrorHandling:
    ...
    async def test_concurrent_error_handling(self):
        ...
        tasks = [create_error(f"Error {i}") for i in range(10)]
