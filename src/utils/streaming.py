...
class SSEFormatter:
    ...
    def format_stream_event(event: StreamEvent, event_id: Optional[str] = None) -> str:
        ...
        return SSEFormatter.format_event(
            event_type=event.type, data=event.model_dump(), event_id=event_id
        )

    @staticmethod
    def format_error_event(
        error: Union[Exception, str, ErrorResponse], event_id: Optional[str] = None
    ) -> str:
        """
        ...
        if isinstance(error, ErrorResponse):
            ...
            error_data = ErrorResponse(
                type="error", error=AnthropicError(type=ErrorType.API_ERROR, message=str(error))
            ).model_dump()
        else:
            error_data = ErrorResponse(
                type="error", error=AnthropicError(type=ErrorType.API_ERROR, message=str(error))
        ...
        return SSEFormatter.format_event(
            event_type="error", data=error_data, event_id=event_id
        )
    ...
    def format_done_event(event_id: Optional[str] = None) -> str:
        ...
        """
        return SSEFormatter.format_event(
            event_type="done", data="[DONE]", event_id=event_id
        )
...
class StreamManager:
    ...
    @asynccontextmanager
    async def managed_stream(self, event_generator: AsyncGenerator[StreamEvent, None]) -> AsyncGenerator[AsyncGenerator[str, None], None]:
        """
...
class ClaudeSDKStreamAdapter:
    ...
    async def adapt_claude_stream(
        ...
        initial_content = ContentBlock(type=ContentType.TEXT, text=" ", source=None)
                ...
                if self._is_content_start(chunk):
                    if not content_started:
                        ...
                        yield ContentBlockStartEvent(
                            index=self.content_block_index, content_block=content_block
                        )
...
async def create_sse_stream(
    event_generator: AsyncGenerator[StreamEvent, None], ping_interval: float = 30.0
) -> AsyncGenerator[str, None]:
    """
