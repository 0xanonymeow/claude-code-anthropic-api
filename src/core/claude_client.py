...
class ClaudeClient:
    ...
    async def _get_sdk(self) -> ClaudeSDKClient:
        ...
        if self._sdk is None:
            try:
                ...
                logger.info(
                    "Initializing Claude Code SDK",
                    event="claude_sdk_init_start",
                    config={
                        k: v for k, v in claude_config.items() if k != "path"
                ...
                logger.info(
                    "Claude Code SDK initialized and connected",
    ...
    def _convert_messages_to_prompt(self, messages: List[Message]) -> str:
        ...
        for message in messages:
            role_prefix = (
                "Human: " if message.role == MessageRole.USER else "Assistant: "
            )
            ...
            if isinstance(message.content, str):
                ...
                for block in message.content:
                    if (
                        ...
                        and block.type == ContentType.TEXT
                    ):
    ...
    def _translate_response_from_claude(
        self, claude_response: Dict[str, Any], original_request: MessageRequest
    ) -> MessageResponse:
        """
        ...
        if isinstance(claude_content, str):
            content_blocks.append(
                ContentBlock(type=ContentType.TEXT, text=claude_content, source=None)
            )
            ...
            for block in claude_content:
                if isinstance(block, dict):
                    ...
                    content_blocks.append(
                        ContentBlock(
                            type=ContentType.TEXT, text=str(block), source=None
                        )
                    )
        ...
        if not content_blocks:
            content_blocks.append(
                ContentBlock(type=ContentType.TEXT, text=" ", source=None)
            )
    ...
    def _map_claude_error_to_anthropic(self, error: Exception) -> ErrorResponse:
        ...
        if isinstance(error, CLINotFoundError):
            anthropic_error = AnthropicError(
                type=ErrorType.NOT_FOUND_ERROR, message="Claude Code CLI not found"
            )
        elif isinstance(error, CLIConnectionError):
            anthropic_error = AnthropicError(
                type=ErrorType.API_ERROR, message="Failed to connect to Claude Code CLI"
            )
            ...
            anthropic_error = AnthropicError(
                type=ErrorType.API_ERROR, message=f"Claude Code SDK error: {str(error)}"
            )
        else:
            anthropic_error = AnthropicError(
                type=ErrorType.API_ERROR, message=f"Unexpected error: {str(error)}"
            )

        return ErrorResponse(type="error", error=anthropic_error)
        ...
        try:
            ...
            if request.model not in self._model_mapping:
                error_response = create_invalid_model_error(
                    request.model, list(self._model_mapping.keys())
                )
                raise ValueError(error_response.error.message)

            logger.log_claude_sdk_request(
                "create_message",
                request.model,
                {
                    "max_tokens": request.max_tokens,
            ...
            logger.log_claude_sdk_response(
                "create_message",
                request.model,
                duration,
                True,
                {"message_id": response.id, "tokens_used": response.usage.total_tokens},
            )
    ...
    async def create_message_stream(
        ...
        try:
            ...
            logger.log_streaming_event(
                "message_start",
                message_id,
                {"model": request.model, "input_tokens": input_tokens},
            )
            ...
            logger.log_streaming_event(
                "message_delta", message_id, {"final_usage": usage.model_dump()}
            )
            ...
            error_response = self._map_claude_error_to_anthropic(e)
            error_event = {"type": "error", "error": error_response.error.model_dump()}
