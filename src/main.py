...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.error(
            "Failed to initialize Claude Code SDK - continuing startup",
...
async def health_check():
    ...
    if not settings.health_check_enabled:
        ...
        return JSONResponse(
            status_code=404, content={"detail": "Health check endpoint is disabled"}
        )
    ...
    try:
        ...
        overall_status = (
            "healthy" if claude_health["status"] == "healthy" else "unhealthy"
        )
        ...
        health_data = {
            ...
            "timestamp": datetime.now(timezone.utc).isoformat() + "Z",
            "components": {
                "claude_code_sdk": {"status": "unhealthy", "error": str(e)},
                "api_server": {"status": "healthy"},
            },
            "error": str(e),
        }
...
async def metrics():
    ...
    if not settings.metrics_enabled:
        ...
        return JSONResponse(
            status_code=404, content={"detail": "Metrics endpoint is disabled"}
        )
    ...
    try:
        ...
        metrics_data = {
            ...
            "claude_code_sdk": {
                ...
                "models_available": claude_health.get("models_available", 0),
            },
            "server": {
                "status": "healthy",
                "uptime": "running",
        ...
        metrics_data = {
            ...
            "timestamp": datetime.now(timezone.utc).isoformat() + "Z",
            "error": str(e),
            "claude_code_sdk": {"status": "unhealthy"},
        }

        return JSONResponse(status_code=503, content=metrics_data)
